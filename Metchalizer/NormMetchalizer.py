...
class NormMetchalizer:
    ...
    def __init__(self, raw_data, batch_with_IDs, prefix_IS= 'IS_', prefix_QC_samples = 'recipe', 
                     remove_outliers_Z_threshold = 3, interia_per_threshold = 85,lambda_1 = 0.5 ,lambda_2 = 1 ):
        ...
        if( len(self.IS_cols) == 0 ):
            raise ValueError("The columns should contain internal standard(s) indicates by "+prefix_IS+". ")
    ...
    def normalize(self):
        ...
        # Select IS data
        X_predict_ = data.loc[:, self.IS_cols ]

        # Scale IS data
        X_predict_ = ((X_predict_ - X_predict_.median(axis=0) ) / X_predict_.mad(axis=0) )
        ...
        # PLS 
        le = LabelBinarizer()
        pls = PLSRegression(n_components = X_predict_.shape[1])
        y_pls = le.fit_transform( X_predict_.assign( batch_labels = batch_labels)['batch_labels'] )
        pls.fit(X_predict_, y_pls )
          
        # Obtain (PLS) PC's and store them in X_trans
        X_trans = pls.transform(X_predict_)
        X_trans = pd.DataFrame(X_trans,index =X_predict_.index )
          
        # Determine how many LV's (latent variables) are needed for regression / mixed effect model fit
        inertias = []
        for i in range(0,X_predict_.shape[1]):
            inertia = 0
            for k, ( batch, IDs ) in enumerate( self.batch_with_IDs.items() ):
        ...
        self.n_components_PLS = range(0, X_predict_.shape[1])[ind] + 1
        ...
        X_PLS = pd.DataFrame( pls.transform( X_predict_ ), index = X_predict_.index ,columns = LVs_cols )
        self.pls = pls 
        ...
        for i,metab in enumerate( data.columns ): 
            ...
            X_predict = X_predict.assign(y = y_)
            ...
            if( fit_succes == True ):
                ...
                for grp, grp_data in X_predict.groupby(by='group'):
                    df__ =  (grp_data['y'] - mdf.random_effects[grp].values[0]).reset_index().set_index('index')
                    df__.columns = ['y_batch_corrected']
                    y_IS_correction =  mdf.fe_params['Intercept'] 
                    for LV in LVs_cols:
                        y_IS_correction += mdf.fe_params[LV] * X_predict.loc[grp_data.index,LV] 
         
                    df__ = df__.assign( y_IS_correction = y_IS_correction )
                    df_ = pd.concat( [df_ , df__ ] )

                df_ = df_.join(X_predict[['LV1']])
                df_ = df_.assign(**{str(metab):  (df_['y_batch_corrected'] - df_['y_IS_correction'] + df_['y_IS_correction'].median()) * y_mad + y_median }  )

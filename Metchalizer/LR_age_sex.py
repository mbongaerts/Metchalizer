...
class LR_age_sex:
    def __init__(self, n_jobs=1, polynomials = 3,a= 35,b= 0.1, normalize=False, Z_outlier_threshold = 3 ):

        self.normalize = normalize
        self.n_jobs = n_jobs
        self.LR = LinearRegression(normalize=self.normalize, n_jobs= self.n_jobs, fit_intercept=False)
    ...
    def p_values(self, n_bootstraps = 50, train_size = 0.95  ):
        ...
        for i in range(n_bootstraps):
            ...
            X_train, X_test, y_train, y_test = train_test_split(self.X_train_not_outliers, 
                                                                self.y_train, 
                                                                test_size= 1 - train_size )
          
            
            model = LR_age_sex(n_jobs=10, polynomials = self.polynomials, a = self.a, b= self.b)
            model.fit(X_train, y_train, interaction = self.interaction)

            beta_means = model.LR.coef_
            beta_vars = np.diag( model.cov_beta )
            
            p_vals_fit = pd.Series( Z_p_val( ( beta_means ) / np.sqrt( beta_vars ) ), index  = model.feature_names )
            p_vals_fits.append(p_vals_fit)
            coefs.append(list(model.LR.coef_) )

        # Results from theoretical framework 
        self.p_vals_fits = pd.DataFrame(p_vals_fits)#.assign( N= len(self.not_outlier_IDs) )
        self.mean_coefs =  pd.Series( np.mean( coefs ,axis=0), index = model.X_train_poly.columns )
